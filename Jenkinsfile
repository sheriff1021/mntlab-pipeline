...
pipeline {
	...
	stage('Publish') {
		steps{
			...
			sh "curl -v --user 'admin:admin' --upload-file ./pipeline-volodya-${BUILD_NUMBER}.tar.gz http://localhost:8081/repository/maven-central/pipeline-volodya-${BUILD_NUMBER}.tar.gz"
		}
	}
